...
void DTFAG::DTFAG_INWC_SPMB_DIF_MR (
    ...
    NTTSPMB NTTSPMB;
    std::ofstream DTFAG_SPMB_DIF_MR("./my_print_data/DTFAG_INWC_SPMB_DIF_MR.txt");
    ...
    for(int i=0; i<radix_r1; i++){
        ...
        DTFAG_SPMB_DIF_MR << "v0[" << i << "] = " << v0[i];
        Tw3[i] = v0[i];
        DTFAG_SPMB_DIF_MR << ", Tw3[" << i << "] = " << Tw3[i] << endl;
        //---------------------------------
        if(stage == 3){
            st3_Tw[i] = Tw3[i];
        }
