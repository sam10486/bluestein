...
void DIF_INWC::INWC_Radix8_BU(ZZ &a_r0,ZZ &a_r1,ZZ &a_r2,ZZ &a_r3,ZZ &a_r4,ZZ &a_r5,ZZ &a_r6,ZZ &a_r7){
	ZZ IW_1_8; // W^(N/8)
	ZZ twiddle_r8_nk;
	ZZ data_tmp;
	...
	for(long i=0;i<8;i++){
		for(long j=0;j<8;j++){
		    nk_exp = i * j;
			nk_exp = nk_exp % 8;
			PowerMod(twiddle_r8_nk,IW_1_8,nk_exp, p);
...
void DIF_INWC::DIF_INWC_r4_r2(std::vector<ZZ> &A,
	...
	std::ofstream INWC_DATARECORD("./NWC_PrintData/INWC_R4_R2_SPMB.txt");
	//-------------Bit Reverse--------------
	BitOperate RR, IntToVec, VecToInt;
	long long bit_width = log2(N);
	long long bit_width_s = log(radix)/log(2);
	...
	bn_tmp = 0;
	BC     = 0;
	INWC_DATARECORD << "init load over! \n";
	int tw_degree = 1; // siang
	//need modify to mult by twiddle factor
	for(int s = 0; s < Stage; s++){
		INWC_DATARECORD <<"----------------------------"<<"\n";
		...
		for(int i = 0 ;i < group;i++){
			...
			for(int j = 0;j < radix;j++){
				INWC_DATARECORD <<"----------------------------"<<"\n";
				INWC_DATARECORD <<"i: "<< i << ", j = " << j <<"\n";
				INWC_DATARECORD <<"twiddle factor =  " << factor << std::endl;
				gray_i  = Gray(i,group);
				BC_tmp  = j * group + gray_i;
				...
				INWC_DATARECORD << "BC_tmp = " << BC_tmp << ", length: " << length <<
				", tw_dg: " <<  tw_degree * length << " = " << tw_degree << " * " << length <<"\n";	// TF_record
				INWC_DATARECORD <<"factor_t =  " << factor_t << ", p = " <<  p <<"\n";
				...
				ZZ r4_InvPhi_deg = PowerMod((ZZ)4, s, p);
				...
				if(bn_tmp == 0){	
					...
					switch(s){
						case 0:
							...
							INWC_DATARECORD <<"A_B0R2[" << ma_tmp << "] = " << A_B0R2[ma_tmp] << ", st0_Tw[2] = " << st0_Tw[2] << endl;
							INWC_DATARECORD <<"A_B0R3[" << ma_tmp << "] = " << A_B0R3[ma_tmp] << ", st0_Tw[3] = " << st0_Tw[3] << endl;
							if(!debug) INWC_DATARECORD << "r4_InvPhi_0t_Order = " << r4_InvPhi_0t_Order << endl; else INWC_DATARECORD << "r4_InvPhi_0t_Order = " << r4_InvPhi_deg*0 << endl;
							if(!debug) INWC_DATARECORD << "r4_InvPhi_1t_Order = " << r4_InvPhi_1t_Order << endl; else INWC_DATARECORD << "r4_InvPhi_1t_Order = " << r4_InvPhi_deg*1 << endl;
							if(!debug) INWC_DATARECORD << "r4_InvPhi_2t_Order = " << r4_InvPhi_2t_Order << endl; else INWC_DATARECORD << "r4_InvPhi_2t_Order = " << r4_InvPhi_deg*2 << endl;
							if(!debug) INWC_DATARECORD << "r4_InvPhi_3t_Order = " << r4_InvPhi_3t_Order << endl; else INWC_DATARECORD << "r4_InvPhi_3t_Order = " << r4_InvPhi_deg*3 << endl;   
							if(!debug) INWC_Radix4_BU(A_B0R0[ma_tmp],A_B0R1[ma_tmp],A_B0R2[ma_tmp],A_B0R3[ma_tmp]);
							//---------------------compute for INWC------------------------
							if(!debug) MulMod(r4_InvPhi_0t_dot_IW, r4_InvPhi_0t_Order, st0_Tw[0], p);
							if(!debug) MulMod(r4_InvPhi_1t_dot_IW, r4_InvPhi_1t_Order, st0_Tw[1], p);
							if(!debug) MulMod(r4_InvPhi_2t_dot_IW, r4_InvPhi_2t_Order, st0_Tw[2], p);
							if(!debug) MulMod(r4_InvPhi_3t_dot_IW, r4_InvPhi_3t_Order, st0_Tw[3], p);

							if(!debug) MulMod(r4_InvPhi_0t_dot_IW_dot_InvFour, r4_InvPhi_0t_dot_IW, InvFour, p);
							if(!debug) MulMod(r4_InvPhi_1t_dot_IW_dot_InvFour, r4_InvPhi_1t_dot_IW, InvFour, p);
							if(!debug) MulMod(r4_InvPhi_2t_dot_IW_dot_InvFour, r4_InvPhi_2t_dot_IW, InvFour, p);
							if(!debug) MulMod(r4_InvPhi_3t_dot_IW_dot_InvFour, r4_InvPhi_3t_dot_IW, InvFour, p);
							//-------------------------------------------------------------
							if(!debug) MulMod(A_B0R0[ma_tmp],A_B0R0[ma_tmp],r4_InvPhi_0t_dot_IW_dot_InvFour,p);
							if(!debug) MulMod(A_B0R1[ma_tmp],A_B0R1[ma_tmp],r4_InvPhi_1t_dot_IW_dot_InvFour,p);
							if(!debug) MulMod(A_B0R2[ma_tmp],A_B0R2[ma_tmp],r4_InvPhi_2t_dot_IW_dot_InvFour,p);
							if(!debug) MulMod(A_B0R3[ma_tmp],A_B0R3[ma_tmp],r4_InvPhi_3t_dot_IW_dot_InvFour,p);
							break;
						case 1:
							INWC_DATARECORD <<"A_B0R0[" << ma_tmp << "] = " << A_B0R0[ma_tmp] << ", st1_Tw[0] = " << st1_Tw[0] << endl;
							INWC_DATARECORD <<"A_B0R1[" << ma_tmp << "] = " << A_B0R1[ma_tmp] << ", st1_Tw[1] = " << st1_Tw[1] << endl;
							...
							if(!debug) INWC_Radix4_BU(A_B0R0[ma_tmp],A_B0R1[ma_tmp],A_B0R2[ma_tmp],A_B0R3[ma_tmp]);
							//---------------------compute for INWC------------------------
							if(!debug) MulMod(r4_InvPhi_0t_dot_IW, r4_InvPhi_0t_Order, st1_Tw[0], p);
							if(!debug) MulMod(r4_InvPhi_1t_dot_IW, r4_InvPhi_1t_Order, st1_Tw[1], p);
							if(!debug) MulMod(r4_InvPhi_2t_dot_IW, r4_InvPhi_2t_Order, st1_Tw[2], p);
							if(!debug) MulMod(r4_InvPhi_3t_dot_IW, r4_InvPhi_3t_Order, st1_Tw[3], p);
						...
						case 2:
							...
							if(!debug) INWC_Radix4_BU(A_B0R0[ma_tmp],A_B0R1[ma_tmp],A_B0R2[ma_tmp],A_B0R3[ma_tmp]);
							//---------------------compute for INWC------------------------
							if(!debug) MulMod(r4_InvPhi_0t_dot_IW, r4_InvPhi_0t_Order, st2_Tw[0], p);
							if(!debug) MulMod(r4_InvPhi_1t_dot_IW, r4_InvPhi_1t_Order, st2_Tw[1], p);
							if(!debug) MulMod(r4_InvPhi_2t_dot_IW, r4_InvPhi_2t_Order, st2_Tw[2], p);
							if(!debug) MulMod(r4_InvPhi_3t_dot_IW, r4_InvPhi_3t_Order, st2_Tw[3], p);
					...
					INWC_DATARECORD << "*****after Mul*****" << std::endl;
					INWC_DATARECORD <<"A_B0R0["<<ma_tmp<<"]: "<< A_B0R0[ma_tmp] << endl;
					INWC_DATARECORD <<"A_B0R1["<<ma_tmp<<"]: "<< A_B0R1[ma_tmp] << endl;
					INWC_DATARECORD <<"A_B0R2["<<ma_tmp<<"]: "<< A_B0R2[ma_tmp] << endl;
					INWC_DATARECORD <<"A_B0R3["<<ma_tmp<<"]: "<< A_B0R3[ma_tmp] << endl;
					...
					switch(s){
						case 0:
							...
							if(!debug) MulMod(r4_InvPhi_3t_dot_IW_dot_InvFour, r4_InvPhi_3t_dot_IW, InvFour, p);
							//-------------------------------------------------------------
							if(!debug) MulMod(A_B1R0[ma_tmp],A_B1R0[ma_tmp],r4_InvPhi_0t_dot_IW_dot_InvFour,p);
							if(!debug) MulMod(A_B1R1[ma_tmp],A_B1R1[ma_tmp],r4_InvPhi_1t_dot_IW_dot_InvFour,p);
							if(!debug) MulMod(A_B1R2[ma_tmp],A_B1R2[ma_tmp],r4_InvPhi_2t_dot_IW_dot_InvFour,p);
							if(!debug) MulMod(A_B1R3[ma_tmp],A_B1R3[ma_tmp],r4_InvPhi_3t_dot_IW_dot_InvFour,p);
							break;
						case 1:
							INWC_DATARECORD <<"A_B1R0[" << ma_tmp << "] = " << A_B1R0[ma_tmp] << ", st1_Tw[0] = " << st1_Tw[0] << endl;
							INWC_DATARECORD <<"A_B1R1[" << ma_tmp << "] = " << A_B1R1[ma_tmp] << ", st1_Tw[1] = " << st1_Tw[1] << endl;
	...
	INWC_DATARECORD << std::endl;	
	INWC_DATARECORD << std::endl;	
	INWC_DATARECORD <<"**************************radix_2 part**************************"<< std::endl;	
	// radix-2 FFT compute
	for(int i = 0; i < group ; i++){
		for(int j=0; j < radix; j++){
			INWC_DATARECORD <<"------------------"<< std::endl;	
			...
			ZZ InvTwo;
			InvMod(InvTwo, (ZZ)2, p);
			ZZ r2_InvPhi_0t_dot_IW, r2_InvPhi_0t_dot_IW_dot_InvTwo;
			ZZ r2_InvPhi_1t_dot_IW, r2_InvPhi_1t_dot_IW_dot_InvTwo;
			ZZ r2_InvPhi_0t, r2_InvPhi_1t, r2_InvPhi_2t, r2_InvPhi_3t;
			ZZ r2_InvPhi_0t_Order, r2_InvPhi_1t_Order, r2_InvPhi_2t_Order, r2_InvPhi_3t_Order;
			ZZ r2_InvPhi_deg = PowerMod((ZZ)4, 3, p);
			...
			if(bn_tmp == 0){
				if(j < 2)bn0_bc_tmp = BC_tmp;
				INWC_DATARECORD <<"A_B0R0[" << ma_tmp << "] = " << A_B0R0[ma_tmp] <<", A_B0R1[" << ma_tmp << "] = " << A_B0R1[ma_tmp] << std::endl;
				INWC_DATARECORD <<"A_B0R2[" << ma_tmp << "] = " << A_B0R2[ma_tmp] <<", A_B0R3[" << ma_tmp << "] = " << A_B0R3[ma_tmp] << std::endl;
				if(!debug) INWC_DATARECORD << "r2_InvPhi_0t_Order = " << r2_InvPhi_0t_Order << endl;
				if(!debug) INWC_DATARECORD << "r2_InvPhi_1t_Order = " << r2_InvPhi_1t_Order << endl;
				INWC_Radix2_BU(A_B0R0[ma_tmp],A_B0R1[ma_tmp]);
				INWC_Radix2_BU(A_B0R2[ma_tmp],A_B0R3[ma_tmp]);
				//-----------------------computr for INWC--------------------------
				if(!debug) MulMod(r2_InvPhi_0t_dot_IW, r2_InvPhi_0t_Order, 1, p);
				if(!debug) MulMod(r2_InvPhi_1t_dot_IW, r2_InvPhi_1t_Order, 1, p);

				if(!debug) MulMod(r2_InvPhi_0t_dot_IW_dot_InvTwo, r2_InvPhi_0t_dot_IW, InvTwo, p);
				if(!debug) MulMod(r2_InvPhi_1t_dot_IW_dot_InvTwo, r2_InvPhi_1t_dot_IW, InvTwo, p);
				//-----------------------------------------------------------------
				if(!debug) MulMod(A_B0R0[ma_tmp], A_B0R0[ma_tmp], r2_InvPhi_0t_dot_IW_dot_InvTwo, p);
				if(!debug) MulMod(A_B0R1[ma_tmp], A_B0R1[ma_tmp], r2_InvPhi_1t_dot_IW_dot_InvTwo, p);

				if(!debug) MulMod(A_B0R2[ma_tmp], A_B0R2[ma_tmp], r2_InvPhi_0t_dot_IW_dot_InvTwo, p);
				if(!debug) MulMod(A_B0R3[ma_tmp], A_B0R3[ma_tmp], r2_InvPhi_1t_dot_IW_dot_InvTwo, p);
				INWC_DATARECORD <<" ------after BU compute and Mul-------" << std::endl;
				INWC_DATARECORD <<"A_B0R0["<<ma_tmp<<"]: "<<A_B0R0[ma_tmp]<<"\n";
				INWC_DATARECORD <<"A_B0R1["<<ma_tmp<<"]: "<<A_B0R1[ma_tmp]<<"\n";
				INWC_DATARECORD <<"A_B0R2["<<ma_tmp<<"]: "<<A_B0R2[ma_tmp]<<"\n";
			    ...
			    if(j < 2)bn1_bc_tmp = BC_tmp;
				INWC_DATARECORD <<"A_B1R0[" << ma_tmp << "] = " << A_B1R0[ma_tmp] <<", A_B1R1[" << ma_tmp << "] = " << A_B1R1[ma_tmp] << std::endl;
				INWC_DATARECORD <<"A_B1R2[" << ma_tmp << "] = " << A_B1R2[ma_tmp] <<", A_B1R3[" << ma_tmp << "] = " << A_B1R3[ma_tmp] << std::endl;
			    if(!debug) INWC_DATARECORD << "r2_InvPhi_0t_Order = " << r2_InvPhi_0t_Order << endl;
				if(!debug) INWC_DATARECORD << "r2_InvPhi_1t_Order = " << r2_InvPhi_1t_Order << endl;
				INWC_Radix2_BU(A_B1R0[ma_tmp],A_B1R1[ma_tmp]);
			    INWC_Radix2_BU(A_B1R2[ma_tmp],A_B1R3[ma_tmp]);
				//-----------------------computr for INWC--------------------------
				if(!debug) MulMod(r2_InvPhi_0t_dot_IW, r2_InvPhi_0t_Order, 1, p);
				if(!debug) MulMod(r2_InvPhi_1t_dot_IW, r2_InvPhi_1t_Order, 1, p);
				...
				if(!debug) MulMod(r2_InvPhi_1t_dot_IW_dot_InvTwo, r2_InvPhi_1t_dot_IW, InvTwo, p);
				//-----------------------------------------------------------------
				if(!debug) MulMod(A_B1R0[ma_tmp], A_B1R0[ma_tmp], r2_InvPhi_0t_dot_IW_dot_InvTwo, p);
				if(!debug) MulMod(A_B1R1[ma_tmp], A_B1R1[ma_tmp], r2_InvPhi_1t_dot_IW_dot_InvTwo, p);

				if(!debug) MulMod(A_B1R2[ma_tmp], A_B1R2[ma_tmp], r2_InvPhi_0t_dot_IW_dot_InvTwo, p);
				if(!debug) MulMod(A_B1R3[ma_tmp], A_B1R3[ma_tmp], r2_InvPhi_1t_dot_IW_dot_InvTwo, p);
    ...
    INWC_DATARECORD << "---------------------------------------------------------\n";
	INWC_DATARECORD << "*********************************************************\n";
	INWC_DATARECORD << "                  FINAL!!!!                              \n";
	INWC_DATARECORD << "*********************************************************\n";
	INWC_DATARECORD << "---------------------------------------------------------\n";
	
    int BC_bit_size;
	BC_bit_size = (int)ceil(log2(N/4));
	INWC_DATARECORD << "BC_bit_size: "<< BC_bit_size <<"\n";
	int index_out;
	for(int i = 0; i < group; i++){
    	for(int j = 0;j < radix;j++){
			INWC_DATARECORD << "-------------------------" << std::endl;
    		...
    		AGU_R4(BC,bn_tmp,ma_tmp);
			INWC_DATARECORD << "bn_tmp: " << bn_tmp <<"\n";
			INWC_DATARECORD << "ma_tmp: " << ma_tmp <<"\n";
			index_out = (int)i * ( N / group) + 4 * j ;
			INWC_DATARECORD << "index_out: " << index_out << std::endl;
    		if(bn_tmp == 0){
     		   A[index_out+0] = A_B0R0[ma_tmp];
    		   A[index_out+1] = A_B0R1[ma_tmp];
    		   A[index_out+2] = A_B0R2[ma_tmp];
    		   A[index_out+3] = A_B0R3[ma_tmp];
			   INWC_DATARECORD << "BN: 0 \n";
			   INWC_DATARECORD << "A["<< index_out      << "]: " << A[index_out+0] << " \n";
			   INWC_DATARECORD << "A["<< index_out + 1  << "]: " << A[index_out+1] << " \n";
			   INWC_DATARECORD << "A["<< index_out + 2  << "]: " << A[index_out+2] << " \n";
			   INWC_DATARECORD << "A["<< index_out + 3  << "]: " << A[index_out+3] << " \n";
    		}
    	    else {
	...
	INWC_DATARECORD << "---------------------------------------------------------\n";
	INWC_DATARECORD << "----                  OVER!!!!!!!                   -----\n";
	INWC_DATARECORD << "---------------------------------------------------------\n";
	INWC_DATARECORD << "---------------------------------------------------------\n";
	INWC_DATARECORD.close();
}


void DIF_INWC::DIF_INWC_r16_r2(std::vector<ZZ> &A,std::vector<ZZ> &B0R0,std::vector<ZZ> &B0R1,std::vector<ZZ> &B0R2,std::vector<ZZ> &B0R3,
	...
	std::ofstream INWC_DATARECORD("./NWC_PrintData/INWC_R16_R2_SPMB.txt");
	//-------------Bit Reverse--------------
	BitOperate RR, IntToVec, VecToInt;
	long long bit_width = log2(N);
	long long bit_width_s = log(radix)/log(2);
	...
	//-----------NWC PART-----------------------
	ZZ InvSixTeen;
	ZZ r16_InvPhi_0t_dot_IW	, r16_InvPhi_0t_dot_IW_dot_InvSixTeen	;
	ZZ r16_InvPhi_1t_dot_IW	, r16_InvPhi_1t_dot_IW_dot_InvSixTeen	;
	ZZ r16_InvPhi_2t_dot_IW	, r16_InvPhi_2t_dot_IW_dot_InvSixTeen	;
	ZZ r16_InvPhi_3t_dot_IW	, r16_InvPhi_3t_dot_IW_dot_InvSixTeen	;
	ZZ r16_InvPhi_4t_dot_IW	, r16_InvPhi_4t_dot_IW_dot_InvSixTeen	;
	ZZ r16_InvPhi_5t_dot_IW	, r16_InvPhi_5t_dot_IW_dot_InvSixTeen	;
	ZZ r16_InvPhi_6t_dot_IW	, r16_InvPhi_6t_dot_IW_dot_InvSixTeen	;
	ZZ r16_InvPhi_7t_dot_IW	, r16_InvPhi_7t_dot_IW_dot_InvSixTeen	;
	ZZ r16_InvPhi_8t_dot_IW	, r16_InvPhi_8t_dot_IW_dot_InvSixTeen	;
	ZZ r16_InvPhi_9t_dot_IW	, r16_InvPhi_9t_dot_IW_dot_InvSixTeen	;
	ZZ r16_InvPhi_10t_dot_IW, r16_InvPhi_10t_dot_IW_dot_InvSixTeen	;
	ZZ r16_InvPhi_11t_dot_IW, r16_InvPhi_11t_dot_IW_dot_InvSixTeen	;
	ZZ r16_InvPhi_12t_dot_IW, r16_InvPhi_12t_dot_IW_dot_InvSixTeen	;
	ZZ r16_InvPhi_13t_dot_IW, r16_InvPhi_13t_dot_IW_dot_InvSixTeen	;
	ZZ r16_InvPhi_14t_dot_IW, r16_InvPhi_14t_dot_IW_dot_InvSixTeen	;
	ZZ r16_InvPhi_15t_dot_IW, r16_InvPhi_15t_dot_IW_dot_InvSixTeen	;
	InvMod(InvSixTeen, (ZZ)16, p);
	cout << "W = " << W << ", IW = " << IW <<  ", Phi = " << Phi << ", InvPhi = " << InvPhi << ", Inv_16 = " << InvSixTeen << endl;
	cout << "p = " << p << endl;
	//------------------------------------------
				...
				if(bn_tmp == 0){
					...
					switch(s){
						case 0:
							...
							if(!debug) MulMod(r16_InvPhi_0t_dot_IW_dot_InvSixTeen, r16_InvPhi_0t_dot_IW, InvSixTeen, p);
							if(!debug) MulMod(r16_InvPhi_1t_dot_IW_dot_InvSixTeen, r16_InvPhi_1t_dot_IW, InvSixTeen, p);
							if(!debug) MulMod(r16_InvPhi_2t_dot_IW_dot_InvSixTeen, r16_InvPhi_2t_dot_IW, InvSixTeen, p);
							if(!debug) MulMod(r16_InvPhi_3t_dot_IW_dot_InvSixTeen, r16_InvPhi_3t_dot_IW, InvSixTeen, p);
							if(!debug) MulMod(r16_InvPhi_4t_dot_IW_dot_InvSixTeen, r16_InvPhi_4t_dot_IW, InvSixTeen, p);
							if(!debug) MulMod(r16_InvPhi_5t_dot_IW_dot_InvSixTeen, r16_InvPhi_5t_dot_IW, InvSixTeen, p);
							if(!debug) MulMod(r16_InvPhi_6t_dot_IW_dot_InvSixTeen, r16_InvPhi_6t_dot_IW, InvSixTeen, p);
							if(!debug) MulMod(r16_InvPhi_7t_dot_IW_dot_InvSixTeen, r16_InvPhi_7t_dot_IW, InvSixTeen, p);
							if(!debug) MulMod(r16_InvPhi_8t_dot_IW_dot_InvSixTeen, r16_InvPhi_8t_dot_IW, InvSixTeen, p);
							if(!debug) MulMod(r16_InvPhi_9t_dot_IW_dot_InvSixTeen, r16_InvPhi_9t_dot_IW, InvSixTeen, p);
							if(!debug) MulMod(r16_InvPhi_10t_dot_IW_dot_InvSixTeen, r16_InvPhi_10t_dot_IW, InvSixTeen, p);
							if(!debug) MulMod(r16_InvPhi_11t_dot_IW_dot_InvSixTeen, r16_InvPhi_11t_dot_IW, InvSixTeen, p);
							if(!debug) MulMod(r16_InvPhi_12t_dot_IW_dot_InvSixTeen, r16_InvPhi_12t_dot_IW, InvSixTeen, p);
							if(!debug) MulMod(r16_InvPhi_13t_dot_IW_dot_InvSixTeen, r16_InvPhi_13t_dot_IW, InvSixTeen, p);
							if(!debug) MulMod(r16_InvPhi_14t_dot_IW_dot_InvSixTeen, r16_InvPhi_14t_dot_IW, InvSixTeen, p);
							if(!debug) MulMod(r16_InvPhi_15t_dot_IW_dot_InvSixTeen, r16_InvPhi_15t_dot_IW, InvSixTeen, p);
							//----------------------------------------------------------------
							MulMod(A_B0R0[ma_tmp],A_B0R0[ma_tmp],r16_InvPhi_0t_dot_IW_dot_InvSixTeen,p);
                    		MulMod(A_B0R1[ma_tmp],A_B0R1[ma_tmp],r16_InvPhi_1t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B0R2[ma_tmp],A_B0R2[ma_tmp],r16_InvPhi_2t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B0R3[ma_tmp],A_B0R3[ma_tmp],r16_InvPhi_3t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B0R4[ma_tmp],A_B0R4[ma_tmp],r16_InvPhi_4t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B0R5[ma_tmp],A_B0R5[ma_tmp],r16_InvPhi_5t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B0R6[ma_tmp],A_B0R6[ma_tmp],r16_InvPhi_6t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B0R7[ma_tmp],A_B0R7[ma_tmp],r16_InvPhi_7t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B0R8[ma_tmp],A_B0R8[ma_tmp],r16_InvPhi_8t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B0R9[ma_tmp],A_B0R9[ma_tmp],r16_InvPhi_9t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B0R10[ma_tmp],A_B0R10[ma_tmp],r16_InvPhi_10t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B0R11[ma_tmp],A_B0R11[ma_tmp],r16_InvPhi_11t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B0R12[ma_tmp],A_B0R12[ma_tmp],r16_InvPhi_12t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B0R13[ma_tmp],A_B0R13[ma_tmp],r16_InvPhi_13t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B0R14[ma_tmp],A_B0R14[ma_tmp],r16_InvPhi_14t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B0R15[ma_tmp],A_B0R15[ma_tmp],r16_InvPhi_15t_dot_IW_dot_InvSixTeen,p);
							break;
						case 1:
							INWC_DATARECORD << "A_B0R0["<< ma_tmp <<"]: " << A_B0R0[ma_tmp]	<< ", st1_Tw[0] = " << st1_Tw[0] << endl;
				    		INWC_DATARECORD << "A_B0R1["<< ma_tmp <<"]: " << A_B0R1[ma_tmp]	<< ", st1_Tw[1] = " << st1_Tw[1] << endl;
					...
					switch(s){
						case 0:
							...
							if(!debug) MulMod(r16_InvPhi_15t_dot_IW_dot_InvSixTeen, r16_InvPhi_15t_dot_IW, InvSixTeen, p);
							//----------------------------------------------------------------
							MulMod(A_B1R0[ma_tmp],A_B1R0[ma_tmp],r16_InvPhi_0t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B1R1[ma_tmp],A_B1R1[ma_tmp],r16_InvPhi_1t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B1R2[ma_tmp],A_B1R2[ma_tmp],r16_InvPhi_2t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B1R3[ma_tmp],A_B1R3[ma_tmp],r16_InvPhi_3t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B1R4[ma_tmp],A_B1R4[ma_tmp],r16_InvPhi_4t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B1R5[ma_tmp],A_B1R5[ma_tmp],r16_InvPhi_5t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B1R6[ma_tmp],A_B1R6[ma_tmp],r16_InvPhi_6t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B1R7[ma_tmp],A_B1R7[ma_tmp],r16_InvPhi_7t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B1R8[ma_tmp],A_B1R8[ma_tmp],r16_InvPhi_8t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B1R9[ma_tmp],A_B1R9[ma_tmp],r16_InvPhi_9t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B1R10[ma_tmp],A_B1R10[ma_tmp],r16_InvPhi_10t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B1R11[ma_tmp],A_B1R11[ma_tmp],r16_InvPhi_11t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B1R12[ma_tmp],A_B1R12[ma_tmp],r16_InvPhi_12t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B1R13[ma_tmp],A_B1R13[ma_tmp],r16_InvPhi_13t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B1R14[ma_tmp],A_B1R14[ma_tmp],r16_InvPhi_14t_dot_IW_dot_InvSixTeen,p);
							MulMod(A_B1R15[ma_tmp],A_B1R15[ma_tmp],r16_InvPhi_15t_dot_IW_dot_InvSixTeen,p);
							break;
						case 1:
							INWC_DATARECORD << "A_B1R0["<< ma_tmp <<"]: " << A_B1R0[ma_tmp]	<< ", st1_Tw[0] = " << st1_Tw[0] << endl;
				    		INWC_DATARECORD << "A_B1R1["<< ma_tmp <<"]: " << A_B1R1[ma_tmp]	<< ", st1_Tw[1] = " << st1_Tw[1] << endl;
	...
	for(int i = 0;i < group; i++){
        for(int j=0; j < radix; j++){
        	...
        	if(bn_tmp == 0){
				...
				if(!debug) MulMod(A_B0R4[ma_tmp], A_B0R4[ma_tmp], r2_InvPhi_0t_dot_IW_dot_InvTwo, p);
				if(!debug) MulMod(A_B0R5[ma_tmp], A_B0R5[ma_tmp], r2_InvPhi_1t_dot_IW_dot_InvTwo, p);

				if(!debug) MulMod(A_B0R6[ma_tmp], A_B0R6[ma_tmp], r2_InvPhi_0t_dot_IW_dot_InvTwo, p);
				if(!debug) MulMod(A_B0R7[ma_tmp], A_B0R7[ma_tmp], r2_InvPhi_1t_dot_IW_dot_InvTwo, p);

				if(!debug) MulMod(A_B0R8[ma_tmp], A_B0R8[ma_tmp], r2_InvPhi_0t_dot_IW_dot_InvTwo, p);
				if(!debug) MulMod(A_B0R9[ma_tmp], A_B0R9[ma_tmp], r2_InvPhi_1t_dot_IW_dot_InvTwo, p);

				if(!debug) MulMod(A_B0R10[ma_tmp], A_B0R10[ma_tmp], r2_InvPhi_0t_dot_IW_dot_InvTwo, p);
				if(!debug) MulMod(A_B0R11[ma_tmp], A_B0R11[ma_tmp], r2_InvPhi_1t_dot_IW_dot_InvTwo, p);

				if(!debug) MulMod(A_B0R12[ma_tmp], A_B0R12[ma_tmp], r2_InvPhi_0t_dot_IW_dot_InvTwo, p);
				if(!debug) MulMod(A_B0R13[ma_tmp], A_B0R13[ma_tmp], r2_InvPhi_1t_dot_IW_dot_InvTwo, p);

				if(!debug) MulMod(A_B0R14[ma_tmp], A_B0R14[ma_tmp], r2_InvPhi_0t_dot_IW_dot_InvTwo, p);
				if(!debug) MulMod(A_B0R15[ma_tmp], A_B0R15[ma_tmp], r2_InvPhi_1t_dot_IW_dot_InvTwo, p);
				...
				if(!debug) MulMod(A_B1R4[ma_tmp], A_B1R4[ma_tmp], r2_InvPhi_0t_dot_IW_dot_InvTwo, p);
				if(!debug) MulMod(A_B1R5[ma_tmp], A_B1R5[ma_tmp], r2_InvPhi_1t_dot_IW_dot_InvTwo, p);

				if(!debug) MulMod(A_B1R6[ma_tmp], A_B1R6[ma_tmp], r2_InvPhi_0t_dot_IW_dot_InvTwo, p);
				if(!debug) MulMod(A_B1R7[ma_tmp], A_B1R7[ma_tmp], r2_InvPhi_1t_dot_IW_dot_InvTwo, p);

				if(!debug) MulMod(A_B1R8[ma_tmp], A_B1R8[ma_tmp], r2_InvPhi_0t_dot_IW_dot_InvTwo, p);
				if(!debug) MulMod(A_B1R9[ma_tmp], A_B1R9[ma_tmp], r2_InvPhi_1t_dot_IW_dot_InvTwo, p);

				if(!debug) MulMod(A_B1R10[ma_tmp], A_B1R10[ma_tmp], r2_InvPhi_0t_dot_IW_dot_InvTwo, p);
				if(!debug) MulMod(A_B1R11[ma_tmp], A_B1R11[ma_tmp], r2_InvPhi_1t_dot_IW_dot_InvTwo, p);

				if(!debug) MulMod(A_B1R12[ma_tmp], A_B1R12[ma_tmp], r2_InvPhi_0t_dot_IW_dot_InvTwo, p);
				if(!debug) MulMod(A_B1R13[ma_tmp], A_B1R13[ma_tmp], r2_InvPhi_1t_dot_IW_dot_InvTwo, p);

				if(!debug) MulMod(A_B1R14[ma_tmp], A_B1R14[ma_tmp], r2_InvPhi_0t_dot_IW_dot_InvTwo, p);
				if(!debug) MulMod(A_B1R15[ma_tmp], A_B1R15[ma_tmp], r2_InvPhi_1t_dot_IW_dot_InvTwo, p);
...
void DIF_INWC::DIF_INWC_r16_r4(std::vector<ZZ> &A,std::vector<ZZ> &B0R0,std::vector<ZZ> &B0R1,std::vector<ZZ> &B0R2,std::vector<ZZ> &B0R3,
	std::vector<ZZ> &B0R4,std::vector<ZZ> &B0R5,std::vector<ZZ> &B0R6,std::vector<ZZ> &B0R7,std::vector<ZZ> &B0R8,std::vector<ZZ> &B0R9,
	std::vector<ZZ> &B0R10,std::vector<ZZ> &B0R11,std::vector<ZZ> &B0R12,std::vector<ZZ> &B0R13,std::vector<ZZ> &B0R14,std::vector<ZZ> &B0R15,
	std::vector<ZZ> &B1R0,std::vector<ZZ> &B1R1,std::vector<ZZ> &B1R2,std::vector<ZZ> &B1R3,std::vector<ZZ> &B1R4,std::vector<ZZ> &B1R5,
	std::vector<ZZ> &B1R6,std::vector<ZZ> &B1R7,std::vector<ZZ> &B1R8,std::vector<ZZ> &B1R9,std::vector<ZZ> &B1R10,std::vector<ZZ> &B1R11,
	...
	std::ofstream DIF_DATARECORD("./NWC_PrintData/INWC_R16_R4_SPMB.txt");
	//**********************************************************
	//**********************************************************
	display  = 1;
	//**********************************************************
	...
	for(int i = 0;i < group; i++){
        for(int j=0; j < radix; j++){
        	...
        	if(bn_tmp == 0){
				...
				if(!debug) MulMod(A_B0R4[ma_tmp],A_B0R4[ma_tmp],r4_InvPhi_0t_dot_IW_dot_InvFour,p);
                if(!debug) MulMod(A_B0R5[ma_tmp],A_B0R5[ma_tmp],r4_InvPhi_1t_dot_IW_dot_InvFour,p);
				if(!debug) MulMod(A_B0R6[ma_tmp],A_B0R6[ma_tmp],r4_InvPhi_2t_dot_IW_dot_InvFour,p);
				if(!debug) MulMod(A_B0R7[ma_tmp],A_B0R7[ma_tmp],r4_InvPhi_3t_dot_IW_dot_InvFour,p);	

				if(!debug) MulMod(A_B0R8[ma_tmp],A_B0R8[ma_tmp],r4_InvPhi_0t_dot_IW_dot_InvFour,p);
                if(!debug) MulMod(A_B0R9[ma_tmp],A_B0R9[ma_tmp],r4_InvPhi_1t_dot_IW_dot_InvFour,p);
				if(!debug) MulMod(A_B0R10[ma_tmp],A_B0R10[ma_tmp],r4_InvPhi_2t_dot_IW_dot_InvFour,p);
				if(!debug) MulMod(A_B0R11[ma_tmp],A_B0R11[ma_tmp],r4_InvPhi_3t_dot_IW_dot_InvFour,p);

				if(!debug) MulMod(A_B0R12[ma_tmp],A_B0R12[ma_tmp],r4_InvPhi_0t_dot_IW_dot_InvFour,p);
                if(!debug) MulMod(A_B0R13[ma_tmp],A_B0R13[ma_tmp],r4_InvPhi_1t_dot_IW_dot_InvFour,p);
				if(!debug) MulMod(A_B0R14[ma_tmp],A_B0R14[ma_tmp],r4_InvPhi_2t_dot_IW_dot_InvFour,p);
				if(!debug) MulMod(A_B0R15[ma_tmp],A_B0R15[ma_tmp],r4_InvPhi_3t_dot_IW_dot_InvFour,p);	
				...
				if(display == 1)DIF_DATARECORD <<"A_B1R14["<<ma_tmp<<"]: "<< A_B1R14[ma_tmp]<<"\n";
				if(display == 1)DIF_DATARECORD <<"A_B1R15["<<ma_tmp<<"]: "<< A_B1R15[ma_tmp]<<"\n";			
        	    INWC_Radix4_BU(A_B1R0[ma_tmp],A_B1R1[ma_tmp],A_B1R2[ma_tmp],A_B1R3[ma_tmp]);
        	    INWC_Radix4_BU(A_B1R4[ma_tmp],A_B1R5[ma_tmp],A_B1R6[ma_tmp],A_B1R7[ma_tmp]);
        	    INWC_Radix4_BU(A_B1R8[ma_tmp],A_B1R9[ma_tmp],A_B1R10[ma_tmp],A_B1R11[ma_tmp]);
        	    INWC_Radix4_BU(A_B1R12[ma_tmp],A_B1R13[ma_tmp],A_B1R14[ma_tmp],A_B1R15[ma_tmp]);
				//------------compute for INWC---------------
				if(!debug) MulMod(r4_InvPhi_0t_dot_IW, r4_InvPhi_0t_Order, 1, p);
				if(!debug) MulMod(r4_InvPhi_1t_dot_IW, r4_InvPhi_1t_Order, 1, p);
				if(!debug) MulMod(r4_InvPhi_2t_dot_IW, r4_InvPhi_2t_Order, 1, p);
				...
				if(!debug) MulMod(A_B1R4[ma_tmp],A_B1R4[ma_tmp],r4_InvPhi_0t_dot_IW_dot_InvFour,p);
                if(!debug) MulMod(A_B1R5[ma_tmp],A_B1R5[ma_tmp],r4_InvPhi_1t_dot_IW_dot_InvFour,p);
				if(!debug) MulMod(A_B1R6[ma_tmp],A_B1R6[ma_tmp],r4_InvPhi_2t_dot_IW_dot_InvFour,p);
				if(!debug) MulMod(A_B1R7[ma_tmp],A_B1R7[ma_tmp],r4_InvPhi_3t_dot_IW_dot_InvFour,p);	

				if(!debug) MulMod(A_B1R8[ma_tmp],A_B1R8[ma_tmp],r4_InvPhi_0t_dot_IW_dot_InvFour,p);
                if(!debug) MulMod(A_B1R9[ma_tmp],A_B1R9[ma_tmp],r4_InvPhi_1t_dot_IW_dot_InvFour,p);
				if(!debug) MulMod(A_B1R10[ma_tmp],A_B1R10[ma_tmp],r4_InvPhi_2t_dot_IW_dot_InvFour,p);
				if(!debug) MulMod(A_B1R11[ma_tmp],A_B1R11[ma_tmp],r4_InvPhi_3t_dot_IW_dot_InvFour,p);

				if(!debug) MulMod(A_B1R12[ma_tmp],A_B1R12[ma_tmp],r4_InvPhi_0t_dot_IW_dot_InvFour,p);
                if(!debug) MulMod(A_B1R13[ma_tmp],A_B1R13[ma_tmp],r4_InvPhi_1t_dot_IW_dot_InvFour,p);
				if(!debug) MulMod(A_B1R14[ma_tmp],A_B1R14[ma_tmp],r4_InvPhi_2t_dot_IW_dot_InvFour,p);
				if(!debug) MulMod(A_B1R15[ma_tmp],A_B1R15[ma_tmp],r4_InvPhi_3t_dot_IW_dot_InvFour,p);	
...
void DIF_INWC::DIF_INWC_r16_r8(std::vector<ZZ> &A,std::vector<ZZ> &B0R0,std::vector<ZZ> &B0R1,std::vector<ZZ> &B0R2,std::vector<ZZ> &B0R3,
	std::vector<ZZ> &B0R4,std::vector<ZZ> &B0R5,std::vector<ZZ> &B0R6,std::vector<ZZ> &B0R7,std::vector<ZZ> &B0R8,std::vector<ZZ> &B0R9,
	std::vector<ZZ> &B0R10,std::vector<ZZ> &B0R11,std::vector<ZZ> &B0R12,std::vector<ZZ> &B0R13,std::vector<ZZ> &B0R14,std::vector<ZZ> &B0R15,
	std::vector<ZZ> &B1R0,std::vector<ZZ> &B1R1,std::vector<ZZ> &B1R2,std::vector<ZZ> &B1R3,std::vector<ZZ> &B1R4,std::vector<ZZ> &B1R5,
	std::vector<ZZ> &B1R6,std::vector<ZZ> &B1R7,std::vector<ZZ> &B1R8,std::vector<ZZ> &B1R9,std::vector<ZZ> &B1R10,std::vector<ZZ> &B1R11,
	...
	display = 1;
	//--------------------------------------------
	std::ofstream DIF_DATARECORD("./NWC_PrintData/INWC_R16_R8_SPMB.txt");
	//-------------Bit Reverse--------------
	BitOperate RR, IntToVec, VecToInt;
	long long bit_width = log2(N);
	long long bit_width_s = log(radix)/log(2);
	...
	for(int i = 0;i < group; i++){
        for(int j=0; j < radix; j++){
        	...
        	if(bn_tmp == 0){
				...
				if(!debug) MulMod(r8_InvPhi_0t_dot_IW_dot_InvEight, r8_InvPhi_0t_dot_IW, InvEight, p);
				if(!debug) MulMod(r8_InvPhi_1t_dot_IW_dot_InvEight, r8_InvPhi_1t_dot_IW, InvEight, p);
				if(!debug) MulMod(r8_InvPhi_2t_dot_IW_dot_InvEight, r8_InvPhi_2t_dot_IW, InvEight, p);
				if(!debug) MulMod(r8_InvPhi_3t_dot_IW_dot_InvEight, r8_InvPhi_3t_dot_IW, InvEight, p);
				if(!debug) MulMod(r8_InvPhi_4t_dot_IW_dot_InvEight, r8_InvPhi_4t_dot_IW, InvEight, p);
				if(!debug) MulMod(r8_InvPhi_5t_dot_IW_dot_InvEight, r8_InvPhi_5t_dot_IW, InvEight, p);
				if(!debug) MulMod(r8_InvPhi_6t_dot_IW_dot_InvEight, r8_InvPhi_6t_dot_IW, InvEight, p);
				if(!debug) MulMod(r8_InvPhi_7t_dot_IW_dot_InvEight, r8_InvPhi_7t_dot_IW, InvEight, p);
				//-------------------------------------------
				if(!debug) MulMod(A_B0R0[ma_tmp],A_B0R0[ma_tmp],r8_InvPhi_0t_dot_IW_dot_InvEight,p);
                if(!debug) MulMod(A_B0R1[ma_tmp],A_B0R1[ma_tmp],r8_InvPhi_1t_dot_IW_dot_InvEight,p);
				if(!debug) MulMod(A_B0R2[ma_tmp],A_B0R2[ma_tmp],r8_InvPhi_2t_dot_IW_dot_InvEight,p);
				if(!debug) MulMod(A_B0R3[ma_tmp],A_B0R3[ma_tmp],r8_InvPhi_3t_dot_IW_dot_InvEight,p);	
				if(!debug) MulMod(A_B0R4[ma_tmp],A_B0R4[ma_tmp],r8_InvPhi_4t_dot_IW_dot_InvEight,p);
                if(!debug) MulMod(A_B0R5[ma_tmp],A_B0R5[ma_tmp],r8_InvPhi_5t_dot_IW_dot_InvEight,p);
				if(!debug) MulMod(A_B0R6[ma_tmp],A_B0R6[ma_tmp],r8_InvPhi_6t_dot_IW_dot_InvEight,p);
				if(!debug) MulMod(A_B0R7[ma_tmp],A_B0R7[ma_tmp],r8_InvPhi_7t_dot_IW_dot_InvEight,p);		

				if(!debug) MulMod(A_B0R8[ma_tmp],A_B0R8[ma_tmp],r8_InvPhi_0t_dot_IW_dot_InvEight,p);
                if(!debug) MulMod(A_B0R9[ma_tmp],A_B0R9[ma_tmp],r8_InvPhi_1t_dot_IW_dot_InvEight,p);
				if(!debug) MulMod(A_B0R10[ma_tmp],A_B0R10[ma_tmp],r8_InvPhi_2t_dot_IW_dot_InvEight,p);
				if(!debug) MulMod(A_B0R11[ma_tmp],A_B0R11[ma_tmp],r8_InvPhi_3t_dot_IW_dot_InvEight,p);	
				if(!debug) MulMod(A_B0R12[ma_tmp],A_B0R12[ma_tmp],r8_InvPhi_4t_dot_IW_dot_InvEight,p);
                if(!debug) MulMod(A_B0R13[ma_tmp],A_B0R13[ma_tmp],r8_InvPhi_5t_dot_IW_dot_InvEight,p);
				if(!debug) MulMod(A_B0R14[ma_tmp],A_B0R14[ma_tmp],r8_InvPhi_6t_dot_IW_dot_InvEight,p);
				if(!debug) MulMod(A_B0R15[ma_tmp],A_B0R15[ma_tmp],r8_InvPhi_7t_dot_IW_dot_InvEight,p);		

					
			}else {
        	    INWC_Radix8_BU(A_B1R0[ma_tmp],A_B1R1[ma_tmp],A_B1R2[ma_tmp],A_B1R3[ma_tmp],A_B1R4[ma_tmp],A_B1R5[ma_tmp],A_B1R6[ma_tmp],A_B1R7[ma_tmp]);
        	    INWC_Radix8_BU(A_B1R8[ma_tmp],A_B1R9[ma_tmp],A_B1R10[ma_tmp],A_B1R11[ma_tmp],A_B1R12[ma_tmp],A_B1R13[ma_tmp],A_B1R14[ma_tmp],A_B1R15[ma_tmp]);
				//------------compute for INWC---------------
				if(!debug) MulMod(r8_InvPhi_0t_dot_IW, r8_InvPhi_0t_Order, 1, p);
				if(!debug) MulMod(r8_InvPhi_1t_dot_IW, r8_InvPhi_1t_Order, 1, p);
				if(!debug) MulMod(r8_InvPhi_2t_dot_IW, r8_InvPhi_2t_Order, 1, p);
				...
				if(!debug) MulMod(r8_InvPhi_7t_dot_IW_dot_InvEight, r8_InvPhi_7t_dot_IW, InvEight, p);
				//-------------------------------------------
				if(!debug) MulMod(A_B1R0[ma_tmp],A_B1R0[ma_tmp],r8_InvPhi_0t_dot_IW_dot_InvEight,p);
                if(!debug) MulMod(A_B1R1[ma_tmp],A_B1R1[ma_tmp],r8_InvPhi_1t_dot_IW_dot_InvEight,p);
				if(!debug) MulMod(A_B1R2[ma_tmp],A_B1R2[ma_tmp],r8_InvPhi_2t_dot_IW_dot_InvEight,p);
				if(!debug) MulMod(A_B1R3[ma_tmp],A_B1R3[ma_tmp],r8_InvPhi_3t_dot_IW_dot_InvEight,p);	
				if(!debug) MulMod(A_B1R4[ma_tmp],A_B1R4[ma_tmp],r8_InvPhi_4t_dot_IW_dot_InvEight,p);
                if(!debug) MulMod(A_B1R5[ma_tmp],A_B1R5[ma_tmp],r8_InvPhi_5t_dot_IW_dot_InvEight,p);
				if(!debug) MulMod(A_B1R6[ma_tmp],A_B1R6[ma_tmp],r8_InvPhi_6t_dot_IW_dot_InvEight,p);
				if(!debug) MulMod(A_B1R7[ma_tmp],A_B1R7[ma_tmp],r8_InvPhi_7t_dot_IW_dot_InvEight,p);		

				if(!debug) MulMod(A_B1R8[ma_tmp],A_B1R8[ma_tmp],r8_InvPhi_0t_dot_IW_dot_InvEight,p);
                if(!debug) MulMod(A_B1R9[ma_tmp],A_B1R9[ma_tmp],r8_InvPhi_1t_dot_IW_dot_InvEight,p);
				if(!debug) MulMod(A_B1R10[ma_tmp],A_B1R10[ma_tmp],r8_InvPhi_2t_dot_IW_dot_InvEight,p);
				if(!debug) MulMod(A_B1R11[ma_tmp],A_B1R11[ma_tmp],r8_InvPhi_3t_dot_IW_dot_InvEight,p);	
				if(!debug) MulMod(A_B1R12[ma_tmp],A_B1R12[ma_tmp],r8_InvPhi_4t_dot_IW_dot_InvEight,p);
                if(!debug) MulMod(A_B1R13[ma_tmp],A_B1R13[ma_tmp],r8_InvPhi_5t_dot_IW_dot_InvEight,p);
				if(!debug) MulMod(A_B1R14[ma_tmp],A_B1R14[ma_tmp],r8_InvPhi_6t_dot_IW_dot_InvEight,p);
				if(!debug) MulMod(A_B1R15[ma_tmp],A_B1R15[ma_tmp],r8_InvPhi_7t_dot_IW_dot_InvEight,p);	
			}			
        }		
	}
